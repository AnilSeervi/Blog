...
const usePrefersColorScheme = () => {
    ...
    const [preferredColorSchema, setPreferredColorSchema] = useState(
        isDark ? 'dark' : isLight ? 'light' : 'light',
    )
    ...
    useEffect(() => {
        ...
        else setPreferredColorSchema('light')
    }, [isDark, isLight])
    ...
    useEffect(() => {
        if (typeof darkQuery?.addEventListener === 'function') {
            ...
            const listener = () => setPreferredColorSchema(darkQuery.matches ? 'dark' : lightQuery.matches ? 'light' : 'light')
